...
void draw() {
  ...
  updatePixels();
  //noLoop();
  px = map(mouseX,0,width,minx,maxx);
  py = map(mouseY,0,height,miny,maxy);
  plotOrbit();
}
  ...
  for (int y = 0; y <height; y++) {
    for (int x =0; x <width; x++) {
      ...
      while (iter < 100) {
        ...
        float aa = a*a - b*b;
        float bb =  2.0 *(a*b);  // us abs(a*b) for burning ship
...
void plotOrbit(){
  ...
  stroke(0,0,100);
  beginShape();
  // get the point on the compex plane
  // copy that into c 
  float ca =px;
  float cb =py;
  // iterate for escape
  for(int i = 0;i<20;i++){
   float tx = map(px,minx,maxx,0,width);
   float ty = map(py,miny,maxy,0,height);
    vertex(tx,ty);
    ellipse(tx,ty,10,10);
    
    
    float aa = px*px - py*py;
    float bb =  2.0 *(px*py);  // use abs(px*py) for burning ship
